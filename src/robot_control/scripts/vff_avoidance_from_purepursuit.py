...
class VFF_Avoidance(Node):
    def __init__(self):
        ...
        self.GREEN = 1
        self.BLUE = 2
        self.create_subscription(Twist, "/cmd_vel_purepursuit", self.cmd_vel_callback, 10)
        self.cmd_vel = Twist()
    ...
    def vff_controller(self):
        ...
        if self.laser_scan != None:
            ...
            vel = Twist()
            vel.linear.x = self.cmd_vel.linear.x
            vel.angular.z = self.cmd_vel.angular.z + max(-0.5, min(0.5, angle)) * 1.2
